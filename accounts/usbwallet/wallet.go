...
type wallet struct {
	...
	deriveChain     embreum.ChainStateReader // Blockchain state reader to discover used account with
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived Embreum account
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain embreum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()

...
import (
	...
	"github.com/Embreum/go-ethereum/core/forkid"
	"github.com/Embreum/go-ethereum/core/types"
	"github.com/Embreum/go-ethereum/les/flowcontrol"
	"github.com/Embreum/go-ethereum/les/utils"
	vfc "github.com/Embreum/go-ethereum/les/vflux/client"
	vfs "github.com/Embreum/go-ethereum/les/vflux/server"
	"github.com/Embreum/go-ethereum/light"
...
type PeerInfo struct {
	Version    int      `json:"version"`    // Embreum protocol version negotiated
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local embreum node is running in archive mode, advertise ourselves we have
...
// participating in the Light Embreum sub-protocol.

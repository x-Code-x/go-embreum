...
// entire API provided by go-embreum  to reduce the maintenance surface and dev
// complexity.
type NodeConfig struct {
	...
	// EthereumEnabled specifies whether the node should run the Embreum protocol.
	EthereumEnabled bool

	// EthereumNetworkID is the network identifier used by the Embreum protocol to
...
// Node represents a Geth Embreum node instance.
type Node struct {
	node *node.Node
}
...
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	// Register the Embreum protocol if requested
	if config.EthereumEnabled {
		...
		if err != nil {
			return nil, fmt.Errorf("embreum init: %v", err)
		}
...
// GetEthereumClient retrieves a client to access the Embreum subsystem.

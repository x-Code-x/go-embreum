...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.embreum", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for go-embreum  issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/embreum/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.embreum", "github.com/embreum/go-embreum/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "embreum", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {

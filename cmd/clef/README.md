...
This setup allows a DApp to connect to a remote Embreum node and send transactions that are locally signed. This can help in situations when a DApp is connected to an untrusted remote Embreum node, because a local one is not available, not synchronised with the chain, or is a node that has no built-in (or limited) account management.
...
GLOBAL OPTIONS:
   ...
   --keystore value        Directory for the keystore (default: "$HOME/.embreum/keystore")
...
The signer will generate a new private key, encrypt it according to [web3 keystore spec](https://github.com/embreum/wiki/wiki/Web3-Secret-Storage-Definition) and store it in the keystore directory.  
...
#### Arguments
  - content type [string]: type of signed data
     ...
     - `application/clique`: [clique](https://github.com/embreum/EIPs/issues/225) headers
...
#### Sign data
   Signs a chunk of structured data conformant to [EIP-712](https://github.com/embreum/EIPs/blob/master/EIPS/eip-712.md) and returns the calculated signature.
...
* A Graphical UI MUST show the blocky-identicon for embreum addresses.
...
| Clef UI| https://github.com/embreum/clef-ui| Golang/QT-based| :+1:| :+1:| :x:| :+1:| :+1:| :x: |  :+1: (approve tx only)|
